...
import {
  Bell, Calendar, Tag, ShieldCheck, CheckCircle2,
  RefreshCw, X, Loader2, Sparkles, MessageSquare
} from 'lucide-react';
...
interface Notification {
  ...
  title: string;
  message: string;
  type: string; // 'booking', 'promo', 'review', etc.
  is_read: boolean;
  created_at: string;
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-0.5">Stay updated with status progress and booking scheduling changes.</p>
        </div>
        ...
        <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl divide-y divide-zinc-850 overflow-hidden shadow-lg">
          {notifications.map((n) => (
            <div
              ...
              className={`p-5 flex items-start gap-4 transition-all duration-150 cursor-pointer ${
                n.is_read ? 'opacity-70 hover:bg-zinc-800/10' : 'bg-indigo-650/5 hover:bg-indigo-650/10'
              }`}
            >
              <div className="w-10 h-10 rounded-xl bg-zinc-850 border border-zinc-800 flex items-center justify-center flex-shrink-0">
                {getIcon(n.type)}
              </div>
              <div className="flex-1 space-y-1">
                <div className="flex items-center justify-between gap-3">
                  <h4 className="text-sm font-bold text-zinc-205 flex items-center gap-1.5">
                    {n.title}
                    {!n.is_read && (
          ...
          {notifications.length === 0 && (
            <div className="py-16 text-center space-y-2">
              <Bell className="w-10 h-10 text-zinc-655 mx-auto" />
              <p className="text-zinc-500 text-sm font-medium">No notifications inboxed at the moment.</p>
            </div>
          )}
        </div>
      )}
