...
export default function UserRatingsPage() {
  ...
  const ratingsFetcher = useCallback(async (url: string) => {
    ...
    return res.data as UserRating[];
  }, []);

  const { data: ratings = [], setData: setRatings, error: fetchError, isLoading } = useSimpleSWR<UserRating[]>(
    '/admin/ratings/users',
    ratingsFetcher
  );
  ...
  const handleDeleteRating = async (id: string) => {
    ...
    try {
      await apiClient.delete(`/admin/ratings/users/${id}`);
    } catch (err) {
      setRatings(originalRatings);
      alert('Failed to delete user rating. Reverting changes.');
    } finally {
  ...
  const tabs = [
    { name: 'User Ratings', href: '/dashboard/admin/ratings/users', active: true },
    { name: 'Handyman Ratings', href: '/dashboard/admin/ratings/handymen', active: false },
  ];
    ...
    <div className="space-y-6 text-zinc-300">
      ...
      <div className="p-3 bg-zinc-900/40 border border-zinc-800/50 backdrop-blur-md rounded-xl flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search ratings by customer name, email, service category, or review content..."
            value={searchQuery}
      ...
      <div className="bg-zinc-900/50 border border-zinc-800/60 backdrop-blur-md rounded-xl overflow-hidden">
        {isLoading ? (
          <div className="p-12 flex flex-col items-center justify-center gap-3">
            ...
            <span className="text-xs text-zinc-550">Fetching user ratings...</span>
          </div>
        ) : fetchError ? (
          <div className="p-12 flex flex-col items-center justify-center gap-3 text-red-400">
            <AlertCircle className="w-8 h-8" />
            <span className="text-xs">Failed to load user ratings list.</span>
          </div>
        ) : filteredRatings.length === 0 ? (
          <div className="p-12 text-center text-zinc-500 text-xs">
            No user ratings found matching your search.
          </div>
        ) : (
