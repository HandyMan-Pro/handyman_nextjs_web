...
import {
  Search, Loader2, AlertCircle, CreditCard, DollarSign, Wallet, ArrowUpRight, TrendingUp, Filter
} from 'lucide-react';
...
export default function PaymentsPage() {
  ...
  const tabs = [
    { name: 'Online Payments', href: '/dashboard/admin/transactions/payments', active: true },
    ...
    <div className="space-y-6 text-zinc-300">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
            <CreditCard className="w-5.5 h-5.5 text-[#5E5CE6]" />
            Financial Engine
          </h1>
          <p className="text-zinc-500 text-xs mt-0.5">
            Monitor, audit, adjust, and reconcile all transactions, cash logs, wallet balances, and withdrawals.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-4 rounded-xl bg-zinc-900/50 border border-zinc-800/60 backdrop-blur-md flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase font-bold text-zinc-500 tracking-wider">Total Online Transactions</span>
        ...
        <div className="p-4 rounded-xl bg-zinc-900/50 border border-zinc-800/60 backdrop-blur-md flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase font-bold text-zinc-500 tracking-wider">Volume Transacted</span>
            <h3 className="text-xl font-bold text-emerald-450 mt-1">${stats.totalVolume.toFixed(2)}</h3>
          </div>
        ...
        <div className="p-4 rounded-xl bg-zinc-900/50 border border-zinc-800/60 backdrop-blur-md flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase font-bold text-zinc-500 tracking-wider">Average Order Value</span>
      ...
      <div className="flex items-center justify-between bg-zinc-900/60 border border-zinc-800/80 p-3 rounded-xl">
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs text-zinc-400 font-semibold">Filter Logs</span>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-xl px-4 py-3 text-xs flex items-center gap-2">
          ...
          <span>Failed to load payments: {fetchError.message || 'Server error.'}</span>
        </div>
      ) : filteredPayments.length === 0 ? (
        <div className="p-12 text-center bg-[#18181b] border border-zinc-850 rounded-xl">
          <span className="text-zinc-500 text-xs font-semibold">No transactions match your filters.</span>
        </div>
      ) : (
