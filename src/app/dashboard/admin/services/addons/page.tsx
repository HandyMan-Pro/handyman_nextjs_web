...
import {
  Blocks, Search, Loader2, AlertCircle, CheckCircle,
  User, RefreshCw, X
} from 'lucide-react';
...
export default function AdminAddonsPage() {
  ...
  const fetchAddons = async () => {
    ...
    setError('');
    try {
      const res = await apiClient.get('/admin/services/addon');
      setAddons(res.data || []);
    } catch (err: any) {
      setError(err.response?.data?.detail || err.message || 'Failed to fetch addon items.');
    } finally {
      setLoading(false);
    }
  };

  const filteredAddons = addons.filter(svc =>
  ...
  const totalEntries = filteredAddons.length;
  const totalPages = Math.ceil(totalEntries / entriesCount) || 1;
  const paginatedAddons = filteredAddons.slice((currentPage - 1) * entriesCount, currentPage * entriesCount);
    ...
    <div className="space-y-6 text-zinc-300">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            Master Addon Services
          </h1>
          <p className="text-zinc-550 text-xs mt-0.5">
            Supervise all optional addon services catalog items offered by providers on the platform.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-zinc-900/60 border border-zinc-800/80 p-3 rounded-xl">
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            type="text"
            placeholder="Search addon name, category, provider..."
            value={searchQuery}
            onChange={(e) => {
        ...
        <div className="text-xs text-zinc-500 font-semibold">
          Showing {filteredAddons.length} addon offerings
        </div>
      </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#5E5CE6]/90 border-b border-zinc-850 text-xs">
                <th className="py-3 px-4 text-left font-bold text-white uppercase tracking-wider">Addon Name</th>
                ...
                <th className="py-3 px-4 text-left font-bold text-white uppercase tracking-wider">Price</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="py-12 text-center text-xs text-zinc-550">
                    No addon services found in catalog.
                  </td>
                </tr>
              ) : (
                paginatedAddons.map(svc => (
                      ...
                      <div className="flex items-center gap-3">
                        {svc.image ? (
                          ...
                          <div className="w-9 h-9 rounded-lg bg-zinc-800 flex items-center justify-center border border-zinc-850">
                            <Blocks className="w-4 h-4 text-zinc-650" />
                          </div>
                        )}
                        <div>
