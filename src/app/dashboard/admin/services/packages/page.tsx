...
import {
  Package, Search, Loader2, AlertCircle, CheckCircle,
  User, RefreshCw, X
} from 'lucide-react';
...
export default function AdminPackagesPage() {
  ...
  const fetchPackages = async () => {
    ...
    setError('');
    try {
      const res = await apiClient.get('/admin/services/package');
      setPackages(res.data || []);
    } catch (err: any) {
      setError(err.response?.data?.detail || err.message || 'Failed to fetch package items.');
    } finally {
      setLoading(false);
    }
  };

  const filteredPackages = packages.filter(svc =>
  ...
  const paginatedPackages = filteredPackages.slice((currentPage - 1) * entriesCount, currentPage * entriesCount);
    ...
    <div className="space-y-6 text-zinc-300">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            Master Package Services
          </h1>
          <p className="text-zinc-550 text-xs mt-0.5">
            Supervise all bundled service packages published by providers for customers on the platform.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-zinc-900/60 border border-zinc-800/80 p-3 rounded-xl">
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            type="text"
            placeholder="Search package name, category, provider..."
            value={searchQuery}
            onChange={(e) => {
        ...
        <div className="text-xs text-zinc-500 font-semibold">
          Showing {filteredPackages.length} package offerings
        </div>
      </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#5E5CE6]/90 border-b border-zinc-850 text-xs">
                <th className="py-3 px-4 text-left font-bold text-white uppercase tracking-wider">Package Name</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="py-12 text-center text-xs text-zinc-550">
                    No package bundles found in catalog.
                  </td>
                </tr>
              ) : (
                paginatedPackages.map(svc => (
