...
filepath = 'src/app/categories/page.tsx'
  ...
  return (
    ...
    >
      ...
      <div style={{ transform: "translateZ(40px)" }} className="absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0 transition-all duration-300 z-20">
        <div className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white shadow-[0_0_20px_rgba(99,102,241,0.6)]">
...
export default function CategoriesPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0c] text-slate-800 dark:text-zinc-100 transition-colors duration-300 font-sans selection:bg-indigo-500/30 flex flex-col">
      
      {/* Navbar */}
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 dark:bg-[#0a0a0c]/80 border-b border-slate-200/50 dark:border-white/5 py-4 px-6 md:px-12 transition-all duration-300">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div 
            className="flex items-center gap-3 cursor-pointer group" 
            onClick={() => router.push('/')}
          >
            <div className="bg-gradient-to-tr from-indigo-600 to-violet-500 p-2.5 rounded-2xl text-white shadow-lg shadow-indigo-600/20 group-hover:scale-105 transition-all duration-300">
              <Wrench className="w-5 h-5" />
            </div>
            <span className="text-xl font-extrabold text-slate-800 dark:text-white tracking-tight">
              Handyman <span className="text-indigo-500">Pro</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {isDarkMode ? <Sun className="w-4.5 h-4.5 text-amber-400 fill-amber-400/20" /> : <Moon className="w-4.5 h-4.5 text-slate-700 fill-slate-700/20" />}
            </button>
            <button 
              onClick={() => router.push('/')}
              className="hidden md:flex items-center gap-2 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-sm font-bold px-5 py-2.5 rounded-2xl transition-all"
            >
              Back to Home
            </button>
          </div>
      ...
      <motion.div 
        ...
        className="relative pt-16 pb-20 px-6 md:px-12 overflow-hidden border-b border-slate-200/50 dark:border-white/5"
      >
        <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-indigo-500/10 rounded-full blur-[120px] pointer-events-none mix-blend-screen hidden dark:block"></div>
        <div className="absolute bottom-[-20%] right-[-10%] w-[500px] h-[500px] bg-purple-500/10 rounded-full blur-[100px] pointer-events-none mix-blend-screen hidden dark:block"></div>
...
print("Updated Categories Page successfully.")
